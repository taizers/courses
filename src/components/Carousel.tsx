...
const Carousel: FC<ThisProps> = ({cards, title = '', autoplay = true, indicators = true, buttons = true}) => {
    ...
    return (
      <div className={'p-2'}>
        {title && (
          <p className="mt-8 text-lg font-medium text-gray-800 dark:text-gray-200">
            {title}
          </p>
        )}
          ...
          >
            {cards.map((card) => (
              <SwiperSlide key={card.id}>
                <div className="flex flex-col items-center bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 lg:w-1/3 w-full">
                  <div className="w-32 h-48 overflow-hidden rounded-lg border-4 border-gray-200 dark:border-gray-700 mb-4">
                    <img
                      src={`${apiImageUrl}${card.pathToImage}`}
