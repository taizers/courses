...
const userMenuItems = [
  ...
  { link: '/admin-courses', label: 'Курсы', access: [adminRole, tutorRole] },
  { link: '/admin-events', label: 'События', access: [adminRole] },
  { link: '/admin-tutors', label: 'Преподаватели', access: [adminRole] },
            ...
            {user?.username ? (
              <div className="relative user-icon">
                ...
                {isUserMenuOpen && (
                  <div
                    ref={userMenuRef}
                    className="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-lg border dark:bg-gray-800 z-20"
                  >
                    <ul className="py-2">
                      {userMenuItems.map((item) => {
                        if (item.access && !item.access.includes(user?.role)) return null;
                        return (
                          <li
                            onClick={() => onUserMenuClick(item)}
                            className="px-4 py-2 text-gray-800 hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 cursor-pointer"
                          >
                            {item.label}
                          </li>
                        );
