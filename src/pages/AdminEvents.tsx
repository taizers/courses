    ...
    return (
        <div className={'py-16 px-6 sm:px-12 max-w-screen-xl mx-auto'}>
            <div className="flex justify-between items-center mb-4">
                ...
                <Button className={'text-black'} onClick={openModal}>Создать событие</Button>
            </div>
            ...
            {!!data?.length && <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400 mx-auto">
                ...
                <tr>
                    ...
                    <th scope="col" className="px-6 py-3">Дата события</th>
                ...
                {data.map((event, index) => (
                    <tr key={index} className="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                        <td className="px-6 py-4">
                            <img width={100} src={event.pathToImage} alt={event.name} />
                        </td>
                        <td className="px-6 py-4">{event.name}</td>
                        <td className="px-6 py-4">{event.description}</td>
                        <td className="px-6 py-4">{moment(event.dateOfEvent).format('DD.MM.YYYY')}</td>
                        <td className="px-6 py-4 flex">
                            <IconButton onClick={() => openEditModal(event)} aria-label="edit">
                                <EditIcon />
                            </IconButton>
                            <IconButton onClick={() => onDelete(event.id)} aria-label="delete">
                                <DeleteIcon />
                            </IconButton>
                        </td>
                    </tr>
