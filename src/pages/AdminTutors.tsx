  ...
  return (
    <div className={'py-16 px-6 sm:px-12 max-w-screen-xl mx-auto'}>
      <div className="flex justify-between items-center mb-4">
        ...
        <Button className={'text-black'} onClick={openModal}>Создать преподавателя</Button>
      </div>
      ...
      {!!data?.length && <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400 mx-auto">
        ...
        <tr>
          ...
          <th scope="col" className="px-6 py-3">Фомилия</th>
          <th scope="col" className="px-6 py-3">Почта</th>
          <th scope="col" className="px-6 py-3">Пароль</th>
          <th scope="col" className="px-6 py-3">Телефон</th>
          <th scope="col" className="px-6 py-3">Возраст</th>
        ...
        {data.map((tutor, index) => (
          <tr key={index} className="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
            ...
            <td className="px-6 py-4 flex">
              <IconButton onClick={() => openEditModal(tutor)} aria-label="edit">
                <EditIcon />
              </IconButton>
              <IconButton onClick={() => changeStatus(tutor.username)} aria-label="changeStatus">
                {tutor.isActive ? <Tooltip title="Block"><BlockIcon /></Tooltip> : <Tooltip title="UnBlock"><CheckIcon /></Tooltip>}
              </IconButton>
              <IconButton onClick={() => onDelete(tutor.id)} aria-label="delete">
                <DeleteIcon />
              </IconButton>
            </td>
          </tr>
